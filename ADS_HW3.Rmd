...
```{r}
#airport name,year, month: # total departing flights, # total distinct destinations, ave length of flight, ave distance of flight, proportion of flights more than 20 minutes late.
tbl(con_air, "flights") %>% 
  group_by(origin, year, month) %>%
  mutate(delayed20 = arr_delay >= 20) %>%
  summarise(total_departing = n_distinct(dest),
            ave_length = mean(air_time),
